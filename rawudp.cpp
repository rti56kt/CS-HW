...
struct dnsheader {
	...
	unsigned char      dnsh_qr:1;
	unsigned char      dnsh_opcode:4;
	unsigned char      dnsh_aa:1;
	unsigned char      dnsh_tc:1;
	unsigned char      dnsh_rd:1;
	unsigned char      dnsh_ra:1;
	unsigned char      dnsh_z:3;
	unsigned char      dnsh_rcode:4;
...
// void dns_domain_name(unsigned char *dname)
// {
...
int main(int argc, char *argv[])
{
	...
	dnsh->dnsh_id = (unsigned short) htons(rand());
	dnsh->dnsh_qr = 0;
	dnsh->dnsh_opcode = 0;
	dnsh->dnsh_aa = 0;
	dnsh->dnsh_tc = 0;
	...
	struct dnsquery *dnsq = (unsigned char * ) (buffer + sizeof(struct ipheader) + sizeof(struct udpheader) + sizeof(struct dnsheader) + sizeof(dnsq_dname));
	dnsq->dnsq_qclass = htons(0x00ff);
	dnsq->dnsq_qtype = htons(0x0001);
